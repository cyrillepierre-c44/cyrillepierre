...
**Security CI steps** : `.github/workflows/ci.yml` rejoue sur chaque push et PR les étapes de `config/ci.rb` (RuboCop, Brakeman, bundler-audit, audit importmap, tests, seeds). Reproduire les échecs en local avec `CI=1 bin/rails test` (eager loading). La CI peut passer au rouge **sans qu'une ligne ait bougé** : `bundler-audit` télécharge la base d'avis à chaque run, et un avis publié dans la journée suffit (16/09/2026 : ReDoS `ruby_llm`, correctif seulement en 2.0.0.rc1). Quand l'avis ne s'applique pas ou que le correctif n'est pas raisonnable, l'ignorer dans `config/bundler-audit.yml` avec la justification **et** la condition de réexamen en commentaire — jamais une entrée nue.
...
la confirmation envoyée à une visiteuse a été perdue sur un `Net::ReadTimeout` dans `do_start`,
sans reprise — elle avait pourtant vu « votre demande a bien été envoyée ». `MailDeliveryJob`
(`config.action_mailer.delivery_job`, déclaré dans `config/application.rb` pour que les tests
couvrent la même classe) rejoue les erreurs **réseau** cinq fois, et **jamais** les erreurs
définitives (authentification, destinataire invalide) : les retenter ne ferait que retarder le
diagnostic. `smtp_settings` porte aussi `open_timeout`/`read_timeout` à 10 s — sans bornes,
`Net::SMTP` occupe un thread SolidQueue très longtemps avant d'abandonner.
...
contenu sans changer leur nom ne sert à rien. Les adresses sans suffixe (`icon-192.png`,
`icon-512.png`) sont **conservées en doublon du contenu à jour** : des appareils gardent en cache,
pour un an, l'ancien manifeste statique qui les désignait, et un 404 leur donnerait une icône vide. Toutes les
...
**redirect_uri** : LinkedIn compare l'adresse de retour au caractère près avec celles déclarées dans « Authorized redirect URLs » de l'app. Rails la fabrique depuis l'hôte de la requête (`linkedin_auth_callback_url`). Depuis la bascule Cloudflare du 09/08/2026, le 301 apex→`www` est rendu en bordure de Cloudflare : Heroku ne voit jamais l'hôte nu, l'adresse est donc toujours `https://www.cyrillepierre.com/auth/linkedin/callback`, la seule à déclarer. La consigne antérieure d'enregistrer aussi la variante sans `www` datait d'avant Cloudflare, quand une requête sur l'apex pouvait atteindre Rails — vérifié le 16/09/2026.
